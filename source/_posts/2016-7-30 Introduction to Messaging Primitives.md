...
One of the most flexible design/architecure techniques is event-driven/message-oriented design.  It offers unparalleled ablity to loosely couple, accomplish Dependency Inversion, facilitates composability, etc.  Message-oriented systems are by nature asynchronous.  This means that a message is sent and the process that sent the message continues on to do other processing without knowning whether the message was received by the other end.  Think of it like sending an email.  You send an email, close you email client, and go on to do something else.  Later, you may return to your email program and notice that the email you sent bounced.  But, you had to return to your email program to see that because you where asynchronously doing something else.

I've been working on an OSS project for a while that provides a framework for simple (once you wrap your head around the degree of loose coupling and asynchronousness) and flexible message-oriented design.  This framework is based on a small set of types that I call primitives.  Before going into what you can do with the framework, let me first cover the primitives.
...
The Command message pattern allows you to encapsulate all the information required to request a handler perform a command, or otherwise change state.  The type of command to be performed is the type of the `ICommand`-implementating class.  Message handlers should typically have no run-time state, so everything the command handler needs to peform the command should included within the `ICommand` type.  For example, if I want to make a request to a handler to create a customer, I may have a `CorrectCustomerAddressCommand` with the following detail:
```csharp
  [Serializable]
  public class CorrectCustomerAddressCommand : ICommand
  {
...
```
As with marker interfaces, we have the ability to introduce metadata when we're defining our messages.  Just like something can be known as a Command because it implements `ICommand`, we can introduce more depth of intent in our messaeges.  For example, if a customer moves, their address changes, but using a command like CorrectCustomerAddress does not include that intent.  We could create a new, identical command named "MoveCustomerCommand" that is effectively identical to `CorrectCustomerAddressCommand` in content (or even derive from the same base class).  And that way we can include semantic intent with the message.  Why would you want to do that?  In the address change example, when a custoner corrects their address they may never have received important mailings.  In the case of a correction, the organization and re-send important mailings.  In the MoveCustomerCommand you may not want to re-send all that information (waste of money, annoys customers) and instead send a card welcoming them to their new home and taking advantage of an opportunity to impress the customer.
##Event
The second message type pattern is an Event.  And just like an event we deal with in day-to-day life: it's a moment in time and information about that moment in time.  From the standpoint of messages, we say that an event is moment of time in the *past*, otherwise known as a fact.  It's important to remember that it's a past fact and really should be considered as immutable data.  We model events in Primitives with the `IEvent` interface, with the following details:
```csharp
...
Because an event is information about a past fact, we call sending an event "publishing" the event.  This concept is considered "pub/sub" (or publish/subscribe) where something that publishes an event never knows how many subscribers (or if any) are subscribed to receive an event.
Typically, when we talk about what we model with events, or facts about the past, we model state changes and include information about that state change.  To correlate to the `CorrectCustomerAddressCommand`; upon a successful address change, the handler of that message may publish a `CustomerAddressCorrectedEvent`.  Which may look like this:
```csharp
  [Serializable]
  public class CustomerAddressCorrectedEvent : IEvent
  {
    public string CorrelationId { get; set; }
...
```
Notice that it's a generic interface and the generic type must implement `IMessage`.  The implementor of `IConsumer<in T>` must also implement a Handle method that takes in the an instance of the message that the class would process.  So, if I wanted to create a class to implement a handler for the `CorrectCustomerAddressCommand` command, it may look like this:
```csharp
  public class CorrectCustomerAddressCommandHandler: IConsumer<CorrectCustomerAddressCommand>
  {
    public void Handle(CorrectCustomerAddressCommand message)
    {
      // ...
    }
  }
```
Now, since we're using interfaces, a class can implement more than one handler.  For example, if I also wanted to process the CustomerAddressCorrectedEvent, I may update my class to be something like:
```csharp
  public class CorrectCustomerAddressCommandHandler : IConsumer<CorrectCustomerAddressCommand>, IConsumer<CustomerAddressChangedEvent>
  {
    public void Handle(CorrectCustomerAddressCommand message)
    {
      // ...
    }
    public void Handle(CustomerAddressChangedEvent message)
    {
      // ...
    }
  }
```
But, as you can tell from the type of the hamdler class (`CorrectCustomerAddressCommandHandler`) that it's named very specifically to be a `CorrectCustomerAddressCommand` handler.  I typically use that convetion and have one handler per classe, which offers a greater flexibility in terms of loosely coupled and composability.  But, in the end, it's up to you what convention you'd like to use.
To write code to handle a particular messages you simply implement `IConsumer<int T>` for one or more types of messages
##Producer
The thing that performs production of a messae is called a Producer, and is modle in Primatives as `IProducer<out T>`, detailed as follows:
```csharp
...
```
Similar to `IConsumer<in T>`, except for the fact the generic type is covariant instead of contravariant and the generic type must implement `IMessage`.
Now this is where the loose coupling and composability takes us into and advanced realm.  You'll notice that the `IProducer` has a single method `AttachConsumer` that accepts an `IConsumer<in T>` where T is the same generic type as the producer.  This is probably very diffrent from a typical imperative design that might have a message that returns a message.  We don't do it that imperitive way because 1) we have message consumption abstraction (`IConsumer`) and 2) the fundamental asynchronousness of messaging.  The production and consumption of messages does not occur in a consistent, sequential fashion such that we would know where to place a call to a method that returns a message.  Instead, we tell the producer who can consume the message and whenever the producer gets around the producing that message, it passes it right along to the consumer.
We may have a class that is a producer of `CorrectCustomerAddressCommandHandler` and could define it as follows:
```csharp
  CorrectCustomerAddressController : IProducer<CorrectCustomerAddressCommandHandler>
  {
    private IConsumer<CorrectCustomerAddressCommandHandler> consumer;
    
    public void AttachConsumer(IConsumer<CorrectCustomerAddressCommandHandler> consumer)
    {
      this.consumer = consumer;
    }
    
    public void CorrectCustomerAddress(Customer customer, Address newAddress)
    {
      if(consumer == null)
        throw new InvalidOperationException(
          "@nameof(consumer) was null during invocation of CorrectCustomerAddress");
          
      var command = new CorrectCustomerAddressCommand()
      {
        CorrelationId = Guid.NewGuid().ToString("N");
        CustomerId = customer.Id;
        PriorAddress = customer.Address;
        Address = newAddress;
      }
...
```
Notice an application-/domain-specific method `CorrectCustomerAddress` method that contains all the information required to send a `CorrectCustomerAddressCommand` (and the `CorrectCustomerAddressCommand` handler would perform the heavy lifting asynchronously and potentially somewhere else, if you're looking for scalability).
You could use this class like this:
```csharp
  var controller = new CorrectCustomerAddressController();
  controller.AttachConsumer(new CorrectCustomerAddressCommandHandler());
```
And then when the `CorrectCustomerAddress` method is called, the consumer is invoked.
##Pipe
...
Typiically though, we use `IPipe` to model various other messaging patterns like [Filters](http://www.enterpriseintegrationpatterns.com/patterns/messaging/Filter.html) (something that would ignore a message based on content) or [Translators](http://www.enterpriseintegrationpatterns.com/patterns/messaging/MessageTranslator.html) (something that converts one message type into another message type).
##Bus
The next message pattern is the Bus.  A bus is modeled with the `IBus` type.  This bus model is basically facilitating implementing a Control Bus.  The Control Bus facilitates connecting message producers with message consumers in a more loosely coupled way.
Remember the `CorrectCustomerAddressController` `IProducer` example?  The code was tightly coupled to both `CorrectCustomerAddressController` and `CorrectCustomerAddressCommandHandler` and we had to new-up both in order to hook them up.  If I'm writing code that produces a message like `CorrectCustomerAddressCommand`, I don't want it to be directly coupled to one particular handler.  After all, we're looking for loosely coupled and asynchronous.  With tight couplling like that I might as well just do all the work in the `CorrectCustomerAddress` method and skip all the messaging.
...
So, if I wanted the type `CorrectCustomerAddressCommandHandler` to handle a `CorrectCustomerAddressCommand` (and produce a `CustomerAddressCorrectedEvent` and a `CustomerAddressCorrectedEventHandler` type to handle a `CustomerAddressCorrectedEvent` message,  could use a mythical bus implementation `Bus` like this:
```csharp
  var bus = new Bus();
  bus.AddHandler(new CorrectCustomerAddressCommandHandler());
  //...
  bus.AddHandler(new CustomerAddressCorrectedEventHandler());
  //...
```
...
```csharp
  bus.Send(new CorrectCustomerAddressCommand());
```
...
```csharp
  ...
  bus.AddHandler(new CustomerAddressCorrectedEventHandler());
  //...
  bus.AddHandler(new CustomerAddressCorrectedEventHandler2());
  ///
```
...
##Idempotency
Another concept that often comes up in messaging is [Idempotency](https://en.wikipedia.org/wiki/Idempotence).  Idempotency is the quality of a message consumer to produce the same side-effects with the same paramters, no matter how many times it is invoked.
What I'm detailed so far with in-memory buses is effectively Reliable Messaging.  If `IBus.Send` returns, the message was reliably processed.  When we start to include message queues and accept messages over the wire, running on multiple computers, we have to deal with the possibility that another server or process might fail and might have to re-send a message.  This typically only happends when the reliability settings of the Queue are not set to the highest level (for example "at-least-once delivery", where the trades performance for the potental a message may be sent more once.  In situations like this you may want to send a message that allows the consumer be an [Idempotent Receiver](http://www.enterpriseintegrationpatterns.com/patterns/messaging/IdempotentReceiver.html).
In our `CorrectCustomerAddressCommand` we've effectively facilitated a Idempotent Receiver, no matter how many times I send a `CorrectCustomerAddressCommand` the resuling customer address will be the same.  Other types of messages make it difficult to have an Idempotent Receiver.  For example, if I had an `IncreaseCustomerAgeCommand`, processing it would always increase an `Age` property of a customer.  If at-least-once-delivery was configured for the queue, this could occaisonally lead to incorect ages.  You may want to either have a command like `SetCustomerAgeCommand` or better yet (avoid pedantry) and have a `CorrectCustomerBirthDateCommand`.
